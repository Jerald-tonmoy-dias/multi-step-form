...
export default function TopNavbar({ navigationTitle }) {
    return (
        <>
            <div className='mx-12 my-8 border rounded-md flex justify-center'>
                {navigationTitle && navigationTitle.map((navigation, index) => {

                    return <button className={`p-4 flex items-center custom-px-40 relative`} key={index}>
                        <span className='text-active_number_color text-4xl font-bold inline-block mr-4'>{index + 1}</span>
                        <span className={`text-md font-bold text-black capitalize`}> {navigation.title}</span>
                        <img className={`absolute right-0 top-0 bottom-0 h-100-percent ${navigationTitle.length - 1 === index ? 'hidden' : ''}`} src={rac} alt="" />
